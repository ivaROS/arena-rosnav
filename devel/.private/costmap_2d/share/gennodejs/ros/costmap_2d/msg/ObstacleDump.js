...
class ObstacleDump {
  ...
  static Resolve(msg) {
    ...
    const resolved = new ObstacleDump(null);
    if (msg.scans !== undefined) {
      resolved.scans = new Array(msg.scans.length);
      for (let i = 0; i < resolved.scans.length; ++i) {
        resolved.scans[i] = sensor_msgs.msg.PointCloud2.Resolve(msg.scans[i]);
      }
    }
    else {
      resolved.scans = []
